function [record,changed] = nt_track_behavior(record,h_dbfig,verbose)
%nt_track_behavior Tracks neurotar experiment from movie and neurotar
%
%  [RECORD,CHANGED] = nt_track_behavior( RECORD, [VERBOSE=true])
%
%     CHANGED is true if markers have been changed
%
%
...
%% Open movies
[handles.vidobj,measures.video_info,active_cameras] = nt_open_videos(record);
record.measures = measures; % to update video info
...
for c = active_cameras
    [video_time,camera(c).time_offset,camera(c).multiplier] = nt_change_times(state.master_time,measures.trigger_times,measures.video_info(c).trigger_times);
...
for c = active_cameras
    t = nt_change_times(handles.vidobj{c}.Duration,measures.video_info(c).trigger_times,measures.trigger_times);
    if t>max_time
        max_time = t;
    end
    t = nt_change_times(0,measures.video_info(c).trigger_times,measures.trigger_times);
    if t<min_time
        min_time = t;
    end
end % c
...
state.jumptime = 0;
state.video_framerate = double(handles.vidobj{1}.FrameRate); % should go to params
...
state.changed = false; % keep track of marker changes by user
    ...
    if state.play || state.newframe
        ...
        if state.play || state.newframe
            ...
            for c = active_cameras
                ...
                % camera_times_in_master_time(c) = nt_change_video_to_neurotar_times( handles.vidobj{c}.CurrentTime, measures.video_info(c).trigger_times, params);
...
if params.nt_show_overhead_mouse
    switch nt_data.Coordinates
        ...
        case params.CAMERA
            [handles.overhead_mouse.XData,handles.overhead_mouse.YData] = ...
                nt_change_camera_to_overhead_coordinates(state.CoM_X *1000,state.CoM_Y*1000,params);
    ...
    switch action % order list alphabetically
        ...
        case 'set_led_position'
            ...
            if ~isempty(putative_trigger_time)
                ...
                logmsg(['Putative trigger time on current timeline: ' num2str(putative_trigger_time - measures.video_info(camera).trigger_times(1)) ' s.']);
            end
            record.measures = measures;
            update_record(record,handles.h_dbfig,true);
            state.changed = true;
...
number_of_frames = ceil(double(vidobj.Duration) * double(vidobj.FrameRate) );
...
actions = {...
    ...
    {'import_markers',           {'i','shift'},'I','Import markers',0},...
